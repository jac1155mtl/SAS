...
/ Purpose   : To insert split characters in a string variable
/ SubMacros : none
...
%if not %length(&var) %then %do;
  %let error=1;
  %put ERROR: (splitvar) No variable name supplied as first positional parameter;
%end;
...
%if %length(&split) NE 3 %then %do;
  %let error=1;
  %put ERROR: (splitvar) Split character &split is not a single character enclosed in quotes;
%end;
