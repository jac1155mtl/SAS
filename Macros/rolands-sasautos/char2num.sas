...
/             macro will create new numeric variables having the same name as the
/             original character variables as well as the same label. You might
/             find the %numchars macro useful for extracting a list of
/             numeric-like character variables. All variables must be specified
/             as a space delimited list. Forms such as char: and char1-char12 are
/             not allowed. No modifiers in brackets are allowed against the input
/             and output data set names.
